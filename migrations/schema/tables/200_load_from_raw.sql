...
SELECT grade_id, grade_code, grade_name, min_salary, max_salary, overtime_eligible
FROM raw.job_grades;

INSERT INTO "$(NS)".job_titles
    (job_id, job_code, job_title, job_family, grade_id, flsa_status)
SELECT job_id, job_code, job_title, job_family, grade_id, flsa_status
FROM raw.job_titles;

INSERT INTO "$(NS)".departments
    (dept_id, dept_code, dept_name, parent_dept_id, cost_center, location_code)
SELECT dept_id, dept_code, dept_name, parent_dept_id, cost_center, location_code
...
SELECT emp_id, emp_number, first_name, last_name, email, hire_date,
       termination_date, dept_id, job_id, manager_emp_id, location_code,
       employment_type, employment_status, active_flag
FROM raw.employees;
...
SELECT salary_id, emp_id, effective_date, end_date, base_salary,
       currency_code, pay_frequency, active_flag
...
SELECT leave_type_id, leave_type_code, leave_type_name, paid_flag,
       accrual_rate, max_balance
...
SELECT balance_id, emp_id, leave_type_id, calendar_year, opening_balance,
       accrued, used, adjustment, pending
FROM raw.leave_balances;

INSERT INTO "$(NS)".pay_elements
    (element_id, element_code, element_name, element_type)
SELECT element_id, element_code, element_name, element_type
...
SELECT period_id, period_name, pay_frequency, period_start_date,
       period_end_date, pay_date, status
...
SELECT run_id, period_id, run_type, run_date, status, total_gross,
       total_deductions, total_net, employee_count
FROM raw.payroll_runs;

INSERT INTO "$(NS)".payroll_details
    (detail_id, run_id, emp_id, element_id, element_type, amount, status)
SELECT detail_id, run_id, emp_id, element_id, element_type, amount, status
